...
class Synth:
    def __init__(self, buffer):
        ...
        self._inserts = dict()
        self._deletes = list()
        self._manifest = json.loads(self._zip_file.open("manifest.json").read().decode(encoding="utf-8"))
...
class LambdaFunction:
    ...
    def absolute_handler_filename(self):
        ...
        return os.path.normpath(os.path.join(self._directory_name, self._raw["Metadata"]["aws:asset:path"], filename))
    ...
    def get_handler(self):
        return self._zip_file.open(self.absolute_handler_filename).read().decode(encoding="utf-8")


class Stack:
    def __init__(self, id, directory_name, raw, zip_file):
        self.id = id
        self.absolute_filename = f"{directory_name}/{raw['properties']['templateFile']}" if directory_name != "" else raw['properties']['templateFile']
        self._directory_name = directory_name
        self._raw = raw
        self._template = json.loads(zip_file.open(self.absolute_filename).read().decode(encoding="utf-8"))
        self._zip_file = zip_file
...
class AssetManifest:
    def __init__(self, id, directory_name, raw, zip_file):
        ...
        self._directory_name = directory_name
        self._raw = raw
        self._manifest = json.loads(zip_file.open(self.absolut_filename).read().decode(encoding="utf-8"))
        self._zip_file = zip_file
    ...
    def replace_hash(self, old, new):
        ...
        cfg["destinations"]["current_account-current_region"]["objectKey"] = f"{new}.zip"
...
class Assembly:
    def __init__(self, id, directory_name, zip_file):
         ...
         self._directory_name = directory_name
         self._zip_file = zip_file
         self._manifest = json.loads(zip_file.open(f"{self._directory_name}/manifest.json").read().decode(encoding="utf-8"))
